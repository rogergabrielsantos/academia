...
function backup_mysql_database($options){
    ...
    $backup_file_name = $options['db_to_backup'] . "sql-backupcu-" . date( "d-m-Y--h-i-s").".sql";
         
    $fp = fopen($backup_file_name ,'w+');
    if (($result = fwrite($fp, $contents))) {
        echo("<script type='text/javascript'> alert('BAckup realizado com sucesso !!!'); location.href='principal.php';</script>");
    }
    fclose($fp);
...
$options = array(
    ...
    'db_uname' => 'root',  //user
    'db_password' => '', //pass
    'db_to_backup' => 'academia', //database name
    'db_backup_path' => 'http://localhost/L2/', //where to backup
